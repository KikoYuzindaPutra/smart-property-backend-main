...
class WaterService {
    ...
    async getAll(dataLogin) {
        ...
        const waterRequest = await TransactionRepository.getAllTransaction("water", dataLogin.id);
        ...
        const water = {
            sent_water_invocie: waterRequest,
            received_water_invoice: managedTransactions
        };
    ...
    * @param {Object} dataWater data tagihan air
    * @param {Object} dataLogin data login user
    * @returns {Promise<Object>}
    */
    async create(dataWater, dataLogin) {
        try {
            const now = Date.now();
            const property = await PropertyRepository.getOnePropertyById(dataWater.property_id);
            ...
            const dataWaterInvoice = {
                property_id: property.id,
                transaction_type: "water",
                nominal: dataWater.nominal,
                transaction_date: now,
                photo: dataWater.photo,
                created_by: dataLogin.id,
            };

            return await TransactionRepository.createTransaction(dataWaterInvoice);
    ...
    async update(dataLogin, _, transactionId) {
        try {
            ...
            if (!transaction || transaction.transaction_type !== "water" || transaction.verified_by) {
